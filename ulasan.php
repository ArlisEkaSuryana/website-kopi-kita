<?php 
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['tambah_ulasan'])) { 
        ...
        if ($stmt->execute()) { 
            $message = "<div class='alert alert-success'>Ulasan dari **$nama** berhasil ditambahkan!</div>"; 
        } else { 
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_ulasan'])) { 
        ...
        if ($stmt->execute()) { 
            $message = "<div class='alert alert-success'>Ulasan dari **$nama** berhasil diperbarui!</div>"; 
        } else { 
            $message = "<div class='alert alert-danger'>Error: " . $stmt->error . "</div>"; 
        } 
        $stmt->close(); 
...
<?php if ($action == 'list'): ?> 
<a href="?action=add" class="btn mb-3" style="background-color:#388e9bff; color:white;">Tambah Ulasan Baru</a> 
        ...
        <tbody> 
            ...
            <tr> 
                ...
                <td> 
                    ...
                    <a href="?action=delete&id=<?php echo $review['id'] ?>" class="btn btn-sm btn-danger" 
...
<h3 class="mt-4"><?php echo $action == 'add' ? 'Tambah' : 'Edit' 
?> Ulasan</h3> 
<form method="POST" action="?action=<?php echo $action ?>"> 
    ...
    <div class="form-group"> 
        ...
        <input type="text" class="form-control" id="status" name="status" 
            value="<?php echo $ulasan_data['status'] ?? 'Pelanggan Kopi Kita Aja' ?>" required> 
    </div> 
    <div class="form-group"> 
        <label for="isi_ulasan">Isi Ulasan</label> 
        <textarea class="form-control" id="isi_ulasan" name="isi_ulasan" rows="4" 
            required><?php echo $ulasan_data['isi_ulasan'] ?? '' ?></textarea> 
    </div> 
    <div class="row"> 
    ...
    <button type="submit" name="<?php echo $action == 'add' ? 'tambah_ulasan' : 'edit_ulasan' ?>" 
